...
## Padrões de Qualidade de Código (Refatoração — Abril 2026)

O código-fonte foi refatorado em abril de 2026. **Todas as alterações futuras devem seguir os padrões abaixo.**

### O que foi feito

- **Comentários traduzidos**: todos os comentários `//` em C++ e QML foram traduzidos do português para inglês. Strings de UI via `LanguageManager.tr3()` não foram alteradas.
- **Variáveis renomeadas**: variáveis genéricas (`i`, `j`, `k`, `b`, `v`) renomeadas para nomes descritivos (`campoIdx`, `boutIdx`, `frameIdx`, `videoPathClean`, etc.) em funções de lógica complexa.
- **Doxygen em C++**: comentários `///` adicionados a métodos públicos (`Q_INVOKABLE`, sinais, slots públicos).
- **Ordering de includes**: organizado como local → Qt (alfabético) → std (alfabético).
- **Mojibake eliminado**: comentários com double-encoding UTF-8 (`â"€â"€`, `Ã§Ã£`, etc.) foram corrigidos.

### Regras Obrigatórias para Futuras Alterações

| Regra | Detalhe |
|---|---|
| **Comentários em inglês** | Sem português em `//`. Strings visíveis ao usuário continuam via `LanguageManager.tr3()`. |
| **Nomes semânticos em loops** | Sem `i`, `j`, `k` em loops não-triviais. Usar `campoIdx`, `boutIdx`, `frameIdx`, `metaIdx`, etc. |
| **Doxygen em APIs públicas C++** | `Q_INVOKABLE`, sinais e slots públicos devem ter comentário `///` antes da declaração. |
| **Ordering de includes** | Local → Qt (alfabético) → std (alfabético). |
| **Emoji em QML via unicode escapes** | Usar `"\u{1F527}"` — nunca emoji literal em strings QML (causa corrupção ao editar). |
| **JSON keys preservados** | Chaves como `"exploração"`, `"distancia_total_m"` não podem ser renomeadas — estão em arquivos de dados existentes no disco. |
| **Novo .qml em `shared/` exige qmldir** | Sempre adicionar entrada em `qml/shared/qmldir` ao criar qualquer componente novo. |

### ⚠️ Atenção: Substituição em Lote de Linhas

A refatoração foi feita com scripts PowerShell de substituição por índice de linha. **Não use essa técnica para renomear variáveis em código complexo** — ela pode inserir novas linhas sem remover as originais, gerando duplicatas e erros de sintaxe. Isso ocorreu em `SessionResultDialog.qml` (detectado e corrigido em 2026-04-28). Para alterações de lógica, use o tool `Edit` com `old_string`/`new_string` exatos.
